...
void Graph<T>::print_matrix()
{
	if (!is_empty())
	{
		...
		//std::cout << "'÷èñëî' - âåñ ðåáðà\n'#' - âåðøèíû â ýòîì íàïðàâëåíèè íå ñâÿçàíû\n'?' - âåðøèíû ñâÿçàíû, íî âåñ ðåáðà íåèçâåñòåí\n\n";
		int width = get_width();
		print_str(width);
...
{
	std::cout << "Вершина " << start_vertex << " пройдена\n";
...
{
	std::cout << "Вершина" << current_vertex << " пройдена \n";
...
void Graph<T>::breadth_first_search(const std::string& start_vertex)
{
	...
	while (!q.empty())
	{
		...
		std::cout << "Вершина " << current_vertex.name << " пройдена\n";
...
void Graph<T>::Dijkstra(const std::string& start_vertex)
{
	...
	{
		std::cout << "От вершины " << vertexes[0].name << " до" << vertexes[i].name << "  = " << vertexes[i].vertex_weight << "\n";
	}
}
